...
db.query(createUserTable, { caching: caching.SKIP }, function (err, res) {
  if (err) throw err;
  if (res.warningCount !== 0) logger.debug("User table already exists");
  else logger.debug("Created new user table");
});

db.query(createSessionTable, { caching: caching.SKIP }, function (err, res) {
  if (err) throw err;
  if (res.warningCount !== 0) logger.debug("Session table already exists");
  else logger.debug("Created new session table");
});
      ...
      .then((response) => {
        ...
        for (const session of response[0]) {
          if (session["endTime"] - session["startTime"] < parseInt(process.env.MAX_TIME)) {
            ...
            if (((Math.floor((session["startTime"] + parseInt(process.env.OFFSET)) / 86400000) + 1) & 7) === parseInt(process.env.MEETING_DAY)) meetings++;
          }
        }
        resolve({
          totalTime: totalTime,
...
function refreshUserCache(password) {
  db.query(mysql.format(getUser, [password]), { hash: "getUser " + password, caching: caching.REFRESH }).catch((error) => {
    logger.error(error);
    return;
  });

  db.query(mysql.format(getUserSessions, [password]), { hash: "getUserSessions " + password, caching: caching.REFRESH }).catch((error) => {
    logger.error(error);
    return;
  });
}
...
function refreshUsersCache() {
  db.query(getUsers, { hash: "getUsers", caching: caching.REFRESH }).catch((error) => {
    logger.error(error);
    return;
  });
}
...
function refreshSessionsCache() {
  db.query(getSessions, { hash: "getSessions", caching: caching.REFRESH }).catch((error) => {
    logger.error(error);
    return;
  });
}
...
router.get("/getuserdata", async (req, res, next) => {
  ...
  const userData: any = await getUserData(req.query.password).catch((error) => {
    logger.error(error);
    res.status(500).send("Something went wrong");
    return;
  });
...
router.get("/getusers", async (req, res, next) => {
  ...
  for (const session of sessions) {
    if (session["endTime"] - session["startTime"] < parseInt(process.env.MAX_TIME)) {
      if (userTimes.has(session["password"])) userTimes.set(session["password"], userTimes.get(session["password"]) + session["endTime"] - session["startTime"]);
      else userTimes.set(session["password"], session["endTime"] - session["startTime"]);

      if (userMeetings.has(session["password"])) {
        if (((Math.floor((session["startTime"] + parseInt(process.env.OFFSET)) / 86400000) + 1) & 7) === parseInt(process.env.MEETING_DAY)) userMeetings.set(session["password"], userMeetings.get(session["password"]) + 1);
      } else userMeetings.set(session["password"], 0);
    }
  }
